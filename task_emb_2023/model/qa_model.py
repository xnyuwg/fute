...
class QaModelV1(BasicModel):
    def __init__(self,
        ...
        if self.config.load_path is None:
            ...
            self.language_model = AutoModelForQuestionAnswering.from_pretrained(self.config.lm_model_name)
        else:
            self.model_load_path = self.config.load_path + '/hf_checkpoint'
            self.language_model = AutoModelForQuestionAnswering.from_pretrained(self.model_load_path, ignore_mismatched_sizes=True, num_labels=2)
