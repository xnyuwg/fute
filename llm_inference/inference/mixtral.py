...
class StoppingCriteriaSub(StoppingCriteria):
    """
    Helper class to config stop token for Mixtral
    """
...
class MixtralInference():
    ...
    def inference(
        ...
        if cot:
            ...
            second_round = self.tokenizer.encode(new_instruction, add_special_tokens=False, return_tensors='pt').to(self.DEVICE)
    ...
    def format_prompt(self, chatgpt_prompt: list) -> torch.Tensor:
        ...
        chatgpt_prompt = chatgpt_prompt[1:]
            ...
            if idx % 2 == 0:
                mixtral_prompt = torch.concat((
                    ...
                    self.tokenizer.encode(
                        f"{B_INST} {content_dict['content'].strip()} {E_INST}",
                        add_special_tokens=False,
                        return_tensors='pt'
                    )
                ), dim=-1)
                ...
                temp_prompt = self.tokenizer.encode(
                    f"{content_dict['content'].strip()}",
                    add_special_tokens=False,
                    return_tensors='pt',
                )
